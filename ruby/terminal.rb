...
module RemoteKeyboard
  ...
  class SerialInterface
    ...
    attr_reader :last_read, :last_write, :port
  ...
  class Keyboard
    ...
    def driveKeyboard(str)
      if str.is_a?(Symbol)
        code = @unshiftedKeys[str] || @shiftedKeys[str]
        press(code)
        release(code)
      else
    ...
    HostKeyToSlaveTranslation = Hash.new {|h,k| h[k] = k }
    HostKeyToSlaveTranslation.merge!({
      "\e[D" => :l_arrow,
      "\e[C" => :r_arrow,
      "\u007F" => :backspace, # DEL
      "\b" => :backspace,
    ...
    def hostToSlaveThread
      reader = TTY::Reader.new

      while true do
        c = reader.read_char
        driveKeyboard(HostKeyToSlaveTranslation[c])
      end
    end

    def slaveToHostThread
